...
void loop()
{
  ...
  invSampleFreq = deltat;
//  updateIMU(gx, gy, gz, ax, ay, az, mx, my, mz);
  updateWithNoMag(gz, gy, gz, ax, ay, az);
...
void updateWithNoMag(float gx, float gy, float gz, float ax, float ay, float az)
{
    float recipNorm;
  float s0, s1, s2, s3;
...
void updateIMU(float gx, float gy, float gz, float ax, float ay, float az, float mx, float my, float mz)
{
  ...
  if((mx == 0.0f) && (my == 0.0f) && (mz == 0.0f)) {
    updateWithNoMag(gx, gy, gz, ax, ay, az);
    return;
  }
