...
// #define init_yaw -1.571
#define PI 3.141592654

bool zero_orientation_set = false;
bool init_set = false;
float sum[15] = {0}, ave[15] = {0}, all_sum[15] = {0}, last[15] = {0}, num[15][10000000] = {0};
int n[15] = {1};
double my_yaw = 0, prev_yaw = 0, init_yaw = -1.571;
...
float calculate_covariance(float x, int i)
{
    ...
    all_sum[i] = 0;
    for (int j=1; j<=n[i]; ++j)
      all_sum[i] = all_sum[i] + (num[i][j] - ave[i]) * (num[i][j] - ave[i]);
    n[i] = n[i] + 1;
    return all_sum[i] / (n[i] - 1);
}
...
int main(int argc, char** argv)
{
  ...
  ros::Publisher imu_pub = nh.advertise<sensor_msgs::Imu>("/fuso/imu", 50);
  ros::Publisher imu_temperature_pub = nh.advertise<sensor_msgs::Temperature>("/fuso/temperature", 50);
  ros::Subscriber sub_initialPose = nh.subscribe("/initialpose", 1000, initialCallback);
            ...
            while (input.length() >= 28) // while there might be a complete package in input
            {
              ...
              if (data_packet_start != std::string::npos)
              {
                ...
                if ((input.length() >= data_packet_start + 28) && (input.compare(data_packet_start + 26, 2, "\r\n") == 0))  //check if positions 26,27 exist, then test values
                {
                  ...
                  ROS_INFO("seems to be a real data package: long enough and found end characters");
                  ...
                  //Plus Initial Orientation
                  // tf::Matrix3x3(differential_rotation).getRPY(roll, pitch, raw_yaw);
                  // // tf::Matrix3x3(orientation).getRPY(roll, pitch, raw_yaw);
                  // // raw_yaw = tf::getYaw(orientation);
                  // my_yaw = raw_yaw - prev_yaw;
                  // if (my_yaw < - PI)
                  // my_yaw += 2 * PI;
                  // else if (my_yaw > PI)
                  // my_yaw -= 2 * PI;
                  // my_yaw += init_yaw;
                  // // geometry_msgs::Quaternion imu_quat = tf::createQuaternionMsgFromYaw(my_yaw);
                  // geometry_msgs::Quaternion imu_quat = tf::createQuaternionMsgFromRollPitchYaw(roll, pitch, my_yaw);
                  // differential_rotation.setX(imu_quat.x);
                  // differential_rotation.setY(imu_quat.y);
                  // differential_rotation.setZ(imu_quat.z);
                  // differential_rotation.setW(imu_quat.w);
                  // differential_rotation.normalize();
                  // orientation.setX(imu_quat.x);
                  // orientation.setY(imu_quat.y);
                  // orientation.setZ(imu_quat.z);
                  // orientation.setW(imu_quat.w);
                  // orientation.normalize();

                  tf::Matrix3x3(differential_rotation).getRPY(roll, pitch, yaw);
                  ...
                  my_yaw -= 2 * PI;
                  my_yaw += init_yaw;
                  plus_orientation = tf::createQuaternionFromRPY(roll, pitch, my_yaw);
                  differential_rotation = plus_orientation.normalized();
                  ...
                  // imu.linear_acceleration_covariance[0] = calculate_covariance(imu.linear_acceleration.x, 12);
                  // imu.linear_acceleration_covariance[4] = calculate_covariance(imu.linear_acceleration.y, 13);
                  // imu.linear_acceleration_covariance[8] = calculate_covariance(imu.linear_acceleration.z, 14);

                  // imu.angular_velocity_covariance[0] = calculate_covariance(imu.angular_velocity.x, 9);
                  // imu.angular_velocity_covariance[4] = calculate_covariance(imu.angular_velocity.y, 10);
                  // imu.angular_velocity_covariance[8] = calculate_covariance(imu.angular_velocity.z, 11);

                  // imu.orientation_covariance[0] = calculate_covariance(roll, 3);
                  // imu.orientation_covariance[4] = calculate_covariance(pitch, 4);
                  // imu.orientation_covariance[8] = calculate_covariance(yaw, 5); 
